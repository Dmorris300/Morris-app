...
// UK-readable date for customer-facing PDFs — "7 August 2026". Accepts ISO
// (yyyy-mm-dd), a Date, or a pre-formatted string (returned as-is).
const MONTHS = ["January","February","March","April","May","June","July","August","September","October","November","December"];
export function fUKDate(v) {
  if (!v) return "";
  if (v instanceof Date) return `${v.getDate()} ${MONTHS[v.getMonth()]} ${v.getFullYear()}`;
  const s = String(v);
...
export function generateQuotePdf({ data, user, today }) {
  ...
  const todayStr = today || uk(new Date());
  ...
  kvTable(state, [
    ...
    ["Proposed start", uk(data.startDate) || "—"],
    ["Quote reference", ref],
    ["Quote date", uk(data.quoteDate) || todayStr],
    ["Valid until", uk(data.validUntil) || "—"],
  ]);
...
function drawDualSignoff(s, data, user, todayStr) {
  ...
  const boxTop = s.y;
  const cells = [
    { title: "PREPARED BY (CONTRACTOR)", name: data.preparedBy || user?.fullName || "—", sig: data.preparedSignature || user?.signature, date: uk(data.quoteDate) || todayStr },
    { title: "ACCEPTED BY (CLIENT)", name: data.clientAcceptanceName || "—", sig: data.clientAcceptanceSignature, date: uk(data.acceptedDate) || "—" },
  ];
