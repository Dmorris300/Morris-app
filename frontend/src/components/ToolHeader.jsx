...
export function ResultActions({ title, content, toolId, photo, photoCaption }) {
  const { user } = useAuth();
  const [busy, setBusy] = useState(null);
  const reviewMandatory = requiresReview(toolId);
  const [reviewConfirmed, setReviewConfirmed] = useState(false);
  const reviewBlocked = reviewMandatory && !reviewConfirmed;

  const guardedAction = (label, fn) => () => {
    if (reviewBlocked) {
      toast.error(`Please confirm you have reviewed this ${label.toLowerCase()} before sharing.`);
      return;
    }
    return fn();
  };

  const onSave = guardedAction("document", async () => {
    setBusy("save");
    try {
  ...
  const onCopy = guardedAction("document", async () => {
    try { await navigator.clipboard.writeText(content); toast.success("Copied"); } catch { toast.error("Copy failed"); }
  });
  const onPdf = guardedAction("document", () => downloadPdf({ title, content, user, photo, photoCaption }));
  const onEmail = guardedAction("document", () => {
    const subject = encodeURIComponent(title || "Document from Morris");
    const body = encodeURIComponent(content);
    window.location.href = `mailto:?subject=${subject}&body=${body}`;
  });
  const onWhatsAppText = guardedAction("document", () => {
    const text = encodeURIComponent(`${title}\n\n${content}\n\n— Generated by Morris (morrisapp.co.uk)`);
    window.open(`https://wa.me/?text=${text}`, "_blank");
  });
  const onWhatsAppPdf = guardedAction("document", () => {
    downloadPdf({ title, content, user, photo, photoCaption });
    setTimeout(() => {
      const text = encodeURIComponent(`${title}. please find the PDF attached. (Generated by Morris)`);
      window.open(`https://wa.me/?text=${text}`, "_blank");
    }, 400);
  });
  const onSms = guardedAction("document", () => {
    ...
    <>
      {reviewMandatory && (
        <div
          ...
          style={{ border: "1px solid #E8A020", background: "rgba(232,160,32,0.06)" }}
          data-testid="review-required-box"
        >
          <AlertTriangle size={18} className="text-[#E8A020] mt-0.5 flex-shrink-0" />
          <label className="flex items-start gap-3 text-sm text-[#A19D94] cursor-pointer">
            <input
              type="checkbox"
              className="mt-1"
            ...
            />
            <span>
              <span className="text-[#F0EDE8] font-semibold">Review required.</span>{" "}
              Please review this document before sending to ensure all details are accurate and specific to your project.
            </span>
          </label>
        </div>
      )}
      <div className={`flex flex-wrap gap-2 mt-5 ${reviewBlocked ? "opacity-50" : ""}`} data-testid="result-actions">
        <ActionBtn onClick={onSave} icon={<Save size={14} />} label="Save" busy={busy === "save"} testId="action-save" disabled={reviewBlocked} />
        <ActionBtn onClick={onCopy} icon={<Copy size={14} />} label="Copy" testId="action-copy" disabled={reviewBlocked} />
        <ActionBtn onClick={onPdf} icon={<Download size={14} />} label="Download PDF" testId="action-pdf" disabled={reviewBlocked} />
        <ActionBtn onClick={onEmail} icon={<Mail size={14} />} label="Email" testId="action-email" disabled={reviewBlocked} />
        <ActionBtn onClick={onWhatsAppText} icon={<MessageCircle size={14} />} label="WhatsApp text" testId="action-wa-text" disabled={reviewBlocked} />
        <ActionBtn onClick={onWhatsAppPdf} icon={<MessageCircle size={14} />} label="WhatsApp PDF" testId="action-wa-pdf" disabled={reviewBlocked} />
        <ActionBtn onClick={onSms} icon={<MessageSquare size={14} />} label="SMS" testId="action-sms" disabled={reviewBlocked} />
      </div>
      <div
