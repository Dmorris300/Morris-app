...
- ✅ **Site Diary V2** at `/app/site-diary` — dashboard-first daily site management. Dashboard: 5 stat cards (Total entries / Today / This week / With delays / Missing today), favourites, recent entries, templates row, search + project + date-range filters. 11-step wizard modal (Project → Date & Weather → Labour/Crew → Works Completed → Deliveries → Plant & Equipment → Delays/Issues/Site Instructions → Variations & Verbal Instructions → Site Photos → Linked Documents → Review & Generate PDF).
- ✅ **Backend** `backend/site_diary.py` — entries CRUD (soft-delete), templates CRUD (reusable day skeletons), `/api/site-diary/stats` with total/enteredToday/thisWeek/withDelays/missingToday, `/api/site-diary/reference` for weather/wind/rain/delay lists, and `collect_site_diary_attention()` for missing-today alerts.
- ✅ **Command Centre integration**: existing `diary_missing` alert in `command_centre.py` now checks the new `site_diary_entries` collection to prevent duplicate alerts, and routes to `/app/site-diary` instead of the legacy multiuser-site-diary page.
- ✅ **Timeline events**: `documents/save` kind_map already maps `site-diary` → `site_diary_created`, so saving an entry emits the event when linked to a project.
- ✅ **Photo Vault**: full `AttachMedia` integration on Step 9 with per-photo captions carried into the PDF.
- ✅ **PDF renderer** `lib/site-diary-pdf.js` — cover page + "OFFICIAL SITE RECORD" banner + section-numbered content (project details, date & weather, labour, works, deliveries, plant, delays/issues/instructions/H&S/visitors, variations, photos, linked docs, notes, sign-off with drawn signature).
- ✅ **UX polish**: auto-computed totalOperatives from crew + subs, autosave draft in localStorage, drop-down project link auto-fills client/site, "New Diary Entry" button routes via `?projectId=…` deep-link from Command Centre attention items.
- ✅ **Wiring**: `App.js` route `site-diary` → `SiteDiary` component; `GenericToolPage.jsx` redirect for tool.id === "site-diary" → `/app/site-diary` for backwards compatibility with existing `/app/tool/site-diary` deep-links.
- ✅ **Testing**: `testing_agent_v3_fork` iteration_18 — 100% backend (13/13 pytest cases in new `/app/backend/tests/test_site_diary.py`), 100% frontend (all 11 wizard steps, all sub-editors, filters, stats, dashboard). Only note: pre-existing hydration warning from preview instrumentation, not blocking.
